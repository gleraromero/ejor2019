...
def ready_time(instance, path, t0):
	...
	t_max = instance["duration_limit"] if "duration_limit" in instance else T # Route maximum duration

	# Check pickup and deliveries.
	if "requests" in instance: # If instance is pickup and delivery, check precedences.
		R = int((n-2)/2) # Number of requests.
		visited = [0 for i in range(0, R)]
		for k in range(1, len(path)-1):
			v = path[k]
			if v > R and visited[(v-1)%R] == 0: return INFTY # If v is delivery and pick was not visited before, infeasible.
			visited[(v-1)%R] += 1 # Add one visit to the request.
		if any([C != 2 and C != 0 for C in visited]): return INFTY # All accepted requests must have two visits.
